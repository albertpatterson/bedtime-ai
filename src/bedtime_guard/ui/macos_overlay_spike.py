...
OVERLAY_TEXT = (
    "Bedtime Guard Overlay Spike\n"
    "Esc closes this spike.\n"
    "This is a feasibility check for full-screen coverage and topmost behavior."
)
...
class OverlayWindow(QWidget):
    def __init__(self, screen_name: str) -> None:
        super().__init__()
        self.setWindowTitle(f"Bedtime Guard Overlay Spike - {screen_name}")
        ...
        label.setFont(QFont("Helvetica Neue", 28))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm:
        parser.error("pass --confirm to launch the overlay spike")

    app = QApplication(sys.argv)
    app.setApplicationName("Bedtime Guard Overlay Spike")
    ...
    if not windows:
        print("No screens detected; overlay spike did not launch.", file=sys.stderr)
        return 1
